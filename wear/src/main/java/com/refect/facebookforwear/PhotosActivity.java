...
import com.refect.facebookforwear.fragments.PhotoFragment;
 ...
 * What should happen:
 * Activity starts
 * Gets the AlbumId from Intent
 * Sends a signal to mobile asking for pictures for AlbumId
 * BroadcastReceiver starts listening for the incoming signal
 * Creates PhotoFragments as they come in
 */
public class PhotosActivity extends FragmentActivity implements MessageApi.MessageListener, GoogleApiClient.ConnectionCallbacks {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        receiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                PhotoModel photoModel = (PhotoModel) intent.getSerializableExtra("photoModel");
                List<Fragment> allFragments = mPagerAdapter.getFragments();
                allFragments.add(PhotoFragment.newInstance(photoModel, allFragments.size() + 1));
                mPagerAdapter.setFragments(allFragments);

            }
        };

        sendMessage(Utils.WEAR_MESSAGE_PATH, Utils.REQUEST_PHOTOS + albumId);
    }
    ...
    protected void onStart() {
        ...
        LocalBroadcastManager.getInstance(this).registerReceiver((receiver),
                new IntentFilter(Utils.PHOTO_ACTIVITY)
        );
    }
